...
## Combining the plots together (remember to save plot after checking)
```{r}
zipcoderentalplot+zipcoderental2023plot
ggsave("results/problem2plots.png")
```
## Problem 3
...
onedaytotalactivity_df =
  ...
  pivot_longer(
    ...
    values_to = "activity") |>
  group_by(seqn, age, gender, education) |>
  summarise (oneday_activity = sum(activity, na.rm = TRUE))

onedaytotalactivity_df

ggplot(onedaytotalactivity_df, aes(x = age, y=oneday_activity, color = gender)) + 
  geom_point (alpha = 0.6) +
  geom_smooth(se = FALSE) +
  facet_grid (~education) +
  labs (
    title = "24-hour Total Activity vs Age by Gender and Education Level",
    x = "Age",
    y = "Total Activity in a Day (24 hours)",
    color = "Gender")
```

##Short Description (check if graph is supposed to look the same as earlier part)
